...
用于完成小样本增量学习，根据框架对原函数进行了一定的修改。

根据README中作者的要求，BibTex引用格式：
...
class CriterionCWD(nn.Module):
    ...
    def forward(self, preds_S, preds_T):
        ...
        for s_feat, t_feat in zip(preds_S, preds_T): # 传进来的是一整组数据，按不同尺度的，所以分别来求
            n,c,h,w = s_feat.shape
            #import pdb;pdb.set_trace()
            norm_s = self.normalize(s_feat / self.temperature)
            norm_t = self.normalize(t_feat.detach()/self.temperature)     

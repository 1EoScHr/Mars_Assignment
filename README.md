...
接着上述，这之中切割后还有一个步骤，也就是进行“向量嵌入”，但其实我也不知道什么是向量嵌入  
...
涉及到LayerNorm、MLP、Window Attention、Shifted Window Attention  
...
降低后，却再次在几乎第六轮的时候又爆了，为了确认是显存大小问题还是算法中没有及时清空无用显存占用，在每一轮的结束时使用CUDA工具来监测显存占用，效果如下：  
![](pictures/48.png)  

所以可能不是没有释放被占用的显存，而是显存大小的问题。  

总之，swin-transformer的确开销有些大，训练慢（只有1-2it/s，远低于backbone的7-8it/s），对显存的要求大。  
...
在计算中对于损失是十分敏感的，倘若一直不下降就需要对学习率及时调整。  

但是看原代码中给定的学习率调度器，是一个“硬编码”的修改逻辑：  
+ 前几轮warmup增加学习率，逐渐从低到给定的baseLearningRate  
+ 然后逐渐用cosine平滑衰减学习率  
+ 最后几轮固定使用minLearningRate  

可见这是与轮数相关的，因此“动态调整”不怎么好。  

所以可以使用一个动态调整学习率的调度器，几轮不下降就缩小学习率。  

这个想法是很自然的，叫做“ReduceLROnPlateau”策略，官方库中就已经给出了。官方调度器在torch.optim.lr_scheduler.ReduceLROnPlateau中，专门对这种需求。  

使用方法无非初始化优化器、初始化调度器、循环中调用等，按照教程，在run函数的调度器使用即可。  
...
损失图如下：  
![](pictures/plateau.png)  

原因可能是为了保险，使用每5轮没有进展才降低学习率，并且在开始时的学习率调的比较小，导致在后面到了一个不是很优的局部最优。  
